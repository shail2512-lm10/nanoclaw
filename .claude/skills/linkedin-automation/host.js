...
function writeResult(requestId, result) {
...
async function runLinkedInScript(taskType, requestId, payload) {
    ...
    return new Promise((resolve) => {
        const proc = spawn('npx', ['dotenv', '-e', '.env', '--', 'npx', 'tsx', scriptPath], {
            cwd: process.cwd(),
            env: envVars,
...
async function handleGetCampaignStats(requestId, payload) {
    try {
...
// ─── Main IPC Handler ─────────────────────────────────────────────────────────
export async function handleLinkedInIpc(data, _sourceGroup, _isMain, _dataDir) {
    if (!LI_TASK_TYPES.has(data.type))
        return false;
    const { type, requestId, ...payload } = data;
    // Special case: stats don't need a browser
    if (type === 'li_get_campaign_stats') {
        handleGetCampaignStats(requestId, payload);
        return true;
    }
